...
from AIGenMusic.utils import get_notes
from AIGenMusic.network import network_model
...
def train():
    ...
    # 用Checkpoint（检查点）文件在每一个Epoch结束时保存模型的参数（Weights）
...
def prepare_sequences(notes, num_pitch):
    ...
    # 将输入标准化/归一化，归一话可以让之后的优化器（optimizer）更快更好地找到误差最小值
    network_input = network_input / float(num_pitch)

    # 将期望输出转换成 {0, 1} 组成的布尔矩阵，为了配合categorical_crossentropy误差算法使用
